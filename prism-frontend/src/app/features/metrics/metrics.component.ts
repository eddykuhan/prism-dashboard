...
import { MetricEntry, MetricType } from '../../core/models/prism.models';
...
export class MetricsComponent implements OnInit, OnDestroy {
  ...
  private generateMockMetrics(name: string, serviceName: string, endTime: number, duration: number, minValue: number, maxValue: number): MetricEntry[] {
    ...
    const hostNames = ['pod-xyz-123', 'pod-abc-456'];
    ...
    for (let i = 0; i < points; i++) {
      ...
      metrics.push({
        ...
        attributes: {
          ...
          'host.name': host,
          ...(name.includes('duration') ? { 'http.route': '/api/metrics' } : {})
        }
      });
    }
  ...
  private processMetrics(): void {
    ...
    // Extract unique hosts from metrics
    const hostSet = new Set<string>();
    this.rawMetrics.forEach(m => {
      const host = m.attributes?.['host.name'];
      if (host && typeof host === 'string') {
        hostSet.add(host);
      }
    });
    this.hosts = Array.from(hostSet).sort();
  ...
  private updateEndpointData(): void {
    ...
    let httpMetrics = this.rawMetrics.filter(m => 
    ...
    if (this.selectedStatusCode) {
      const statusCode = this.selectedStatusCode.charAt(0);
      httpMetrics = httpMetrics.filter(m => {
        const code = m.attributes?.['http.status_code'] || m.attributes?.['http.response.status_code'];
        return code && code.toString().startsWith(statusCode);
      });
    }
    if (this.selectedHost) {
      httpMetrics = httpMetrics.filter(m => m.attributes?.['host.name'] === this.selectedHost);
    }
  ...
  private updateMainChart(metricName: string): void {
    ...
    if (this.selectedHost) {
      filtered = filtered.filter(m => m.attributes?.['host.name'] === this.selectedHost);
    }
